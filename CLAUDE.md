...
struct PomodoroApp {
    ...
    now: Duration,
    /// 场景交叉淡化器 (含当前场景索引, danqing::Crossfade)。
    fader: Crossfade,
    /// 场景动效沉降补间 (纯逻辑：暂停 500ms 淡出 / 恢复淡入, danqing::Tween)。
...
### 动效沉降补间 (2026-09-09 起下沉框架, 用 `danqing::Tween`)

视觉动效潮汐式沉降 (运行=全量, 暂停=500ms 归零) 已由框架原语承载：
`Tween::new(motion::SETTLE_DURATION, Easing::Linear)`，tick 内 `value(now, target)` 推进。
时间由外部注入 (`Duration` 累计值)，不读 wall-clock。
场景交叉淡化 (`Crossfade`) / 完成脉冲 (`Pulse`) / 首启提示 (`Cue`) 同为框架原语
(spec: danqing `docs/specs/SPEC-anim.md`); motion.rs 只保留场景索引常量与强度合成函数 (见下)。
...
├── motion.rs     ← shader 动效策略 (场景索引常量 + 强度合成; 补间用 danqing::Tween)
├── ambient.rs    ← 环境音程序化生成
