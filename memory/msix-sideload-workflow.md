...
- **任务栏图标蓝色占位块 (2026-09-01 终判)**: 产品侧全链路验证干净 — exe 内嵌图标✓、
  窗口 ICON_SMALL/BIG 探针实测✓、包资产✓、注册清单✓、部署日志✓、图标缓存已清✓、
  任务视图磁贴✓ — 唯读本机任务栏按钮仍蓝。判定: 同 AUMID 反复装卸十余次造成的
  shell 固癖, 非产品缺陷, 商店全新安装不受影响; 不阻塞提交, 上架后以商店签名版复验
